...
contract NFTSwap2 {
    ...
    function deposit(uint256 swapId, address contractAddress, uint256 tokenId) public {
        ...
        require(current.from != address(0x0), "swap not exist");

        if (current.from == msg.sender) {
            current.token1Received = true;
        } else if (current.to == msg.sender) {
    ...
    function cancelSwap(uint256 swapId) public {
        ...
        if (current.token1Received == true) {
            IERC721(current.contract1).safeTransferFrom(address(this), current.from, current.tokenId1);
        }

        if (current.token2Received == true) {
            IERC721(current.contract2).safeTransferFrom(address(this), current.to, current.tokenId2);
        }
    ...
    function acceptSwap(uint256 swapId) public {
        ...
        IERC721(current.contract1).safeTransferFrom(address(this), current.to, current.tokenId1);
        IERC721(current.contract2).safeTransferFrom(address(this), current.from, current.tokenId2);
